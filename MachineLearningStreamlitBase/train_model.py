...
class generateFullData:
    ...
    def trainXGBModel_binaryclass(self, data, feature_names, label_name, replication_set=None):
        ...
        deval = xgb.DMatrix(pd.concat([X_train_valid, X_valid], axis=0),
                            label=pd.concat([y_train_valid, y_valid], axis=0), feature_names=feature_names)
        eval_set = [(deval, 'eval')]
        ...
        for e, param in enumerate(params):
            ...
            model_temp = xgb.train(param, dtrain_train, num_round, evals=eval_set, verbose_eval=False,
        ...
        expected_values = {'explainer_train': explainer_train.expected_value,
                           'explainer_test': explainer_test.expected_value}
